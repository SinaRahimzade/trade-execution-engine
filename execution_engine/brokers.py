...
class MofidBroker(Authentication, ABC):
    ...
    def get_portfolio(self):
        ...
        request_response = requests.get(
            url=self.get_assets_url, headers=headers_template
        )
    ...
    def get_liquidity(self):
        ...
        request_response = requests.get(
            url=self.get_liquidity_url, headers=headers_template
        )
    ...
    def send_order(self, order_type, ticker_isin_code, quantity, price):
        ...
        request_response = requests.post(
            url=self.order_url, headers=headers_template, json=payload_template
        )
    ...
    def market_watch(self, watchlist_name):
        ...
        request_response = json.loads(
            requests.get(url=self.market_watch_url, headers=headers_template).text
        )
...
class ExirBroker(Authentication, ABC):
    def __init__(self, broker_name, username, password):
        ...
        self.get_portfolio_url = (
            "https://{}.exirbroker.com/api/v1/restapi/customers/livePortfolio?size=-1"
        )
    ...
    def account_login(self):
        ...
        try:
            element_present = expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, ".fa.fa-times-circle.close")
            )
            WebDriverWait(self.driver, timeout).until(element_present)
        ...
        close_pop_up_button = self.driver.find_element(
            By.CSS_SELECTOR, ".fa.fa-times-circle.close"
        )
    ...
    def get_portfolio(self):
        ...
        request_response = json.loads(
            requests.get(
                url=self.get_portfolio_url.format(self.broker_name),
                headers=header_template,
            ).text
        )

        portfolio = list(
            filter(lambda asset: asset["remainQty"] > 0, request_response["result"])
        )
    ...
    def get_orders_history(self, starting_date, ending_date):
        ...
        url = self.get_orders_history_url.format(
            broker=self.broker_name, start=starting_date, end=ending_date
        )
    ...
    def market_watch(self, watchlist_name=None):
        ...
        request_response = json.loads(
            requests.post(
                url=self.market_watch_url.format(self.broker_name),
                headers=header_template,
                json={},
            ).text
        )
